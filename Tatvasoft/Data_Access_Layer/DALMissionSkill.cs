...
namespace Data_Access_Layer
{
    public class DALMissionSkill
    {
        ...
        {
            return await _cIDbContext.MissionSkills.Where(x => !x.IsDeleted).ToListAsync();
        }

        public async Task<MissionSkill> GetMissionSkillByIdAsync(int id)
        {
            return await _cIDbContext.MissionSkills.Where(x => !x.IsDeleted && x.Id == id).FirstOrDefaultAsync();
        }
        ...
        public async Task<string> UpdateMissionSkillAsync(MissionSkill missionSkill)
        {
            try
            {
                var existingSkill = await _cIDbContext.MissionSkills.Where(x => !x.IsDeleted && x.Id == missionSkill.Id).FirstOrDefaultAsync();
        ...
        public async Task<string> DeleteMissionSkillAsync(int id)
        {
            try
            {
                var existingSkill = await _cIDbContext.MissionSkills.Where(x => !x.IsDeleted && x.Id == id).FirstOrDefaultAsync();
                if (existingSkill != null)
                {
